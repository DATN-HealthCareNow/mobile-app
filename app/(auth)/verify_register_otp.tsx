...
import { useRegister, useRequestRegisterOtp } from "../../hooks/useAuth";
...
export default function VerifyRegisterOtpScreen() {
  ...
  const { t } = useLanguage();
  const { email, full_name, password } = useLocalSearchParams<{
    email: string;
    full_name: string;
    password: string;
  }>();
  ...
  useEffect(() => {
    if (!email || !password) {
      Alert.alert(
        t("auth.common.error"),
        t("auth.verify_register.invalid_params"),
      );
  ...
  const handleVerify = () => {
    ...
    registerMutation.mutate(
      { full_name, email, password, otp },
      {
        onSuccess: () => {
          Alert.alert(
            t("auth.verify_register.success_title"),
            t("auth.verify_register.success_message"),
            [
              {
                text: t("auth.verify_register.ok"),
                onPress: () => router.replace("/screen/metric_entry"),
              },
            ],
          );
        },
        onError: (err: any) => {
          Alert.alert(
            ...
            err?.response?.data?.message ||
              t("auth.verify_register.create_failed"),
          );
        },
      },
    );
    ...
    >
      ...
      >
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.title}>{t("auth.verify_register.title")}</Text>
        ...
        <View style={styles.form}>
          ...
          <TouchableOpacity
            style={[
              ...
              (otp.length < 6 || registerMutation.isPending) &&
          ...
          >
            {registerMutation.isPending ? (
              ...
              <Text style={styles.buttonText}>
                {t("auth.verify_register.confirm_register")}
              </Text>
            )}
          </TouchableOpacity>
          ...
          <View style={styles.infoBox}>
            ...
            <Text style={styles.infoText}>
              {t("auth.verify_register.security_note")}
            </Text>
          </View>
        </View>
      </ScrollView>
