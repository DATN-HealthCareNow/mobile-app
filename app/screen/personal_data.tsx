...
export default function PersonalDataScreen() {
  ...
  const {
    ...
    isLoading,
    isError,
  } = useDailyHealthMetric(todayDateString);
  ...
  const distanceMeters =
    Number(m.distance_meters ?? m.distanceMeters ?? 0) +
    Number(m.google_distance_meters ?? m.googleDistanceMeters ?? 0);
  const activeCalories =
    Number(m.active_calories ?? m.activeCalories ?? 0) +
    Number(m.google_active_calories ?? m.googleActiveCalories ?? 0);
  ...
  const restingHeartRate = Number(
    m.resting_heart_rate ?? m.restingHeartRate ?? 0,
  );
  const movementMinutes =
    Number(m.google_exercise_minutes ?? m.googleExerciseMinutes ?? 0) +
  ...
  const formatSleepMinutes = (minutes = 0) => {
    ...
    return `${hrs}${t("personal_data.unit_h", "h")} ${mins}${t(
      "personal_data.unit_m",
      "m",
    )}`;
  };
  ...
  const formatDistance = (meters = 0) => {
    if (meters <= 0) return "--";
    if (meters < 1000)
      return `${Math.round(meters)} ${t("personal_data.unit_meter", "m")}`;
    return `${(meters / 1000).toFixed(2)} ${t("personal_data.unit_km", "km")}`;
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>{t("personal_data.title")}</Text>
        <View style={{ width: 44 }} />
      </View>
      ...
      >
        ...
        {isLoading ? (
          ...
          <View style={styles.grid}>
            ...
            <MetricCard
              icon={
                <FontAwesome5 name="shoe-prints" size={20} color="#3b82f6" />
              }
              label={t("personal_data.steps")}
              value={steps > 0 ? steps.toLocaleString() : "--"}
              unit={t("personal_data.unit_steps")}
              colors={colors}
              styles={styles}
            />
            ...
            <MetricCard
              ...
              label={t("personal_data.distance")}
            ...
            <MetricCard
              ...
              label={t("personal_data.heart_rate")}
              value={heartRate > 0 ? heartRate.toString() : "--"}
              unit={t("personal_data.unit_bpm")}
              colors={colors}
              styles={styles}
            />
            ...
            <MetricCard
              ...
              label={t("personal_data.resting_hr")}
            ...
            <MetricCard
              ...
              label={t("personal_data.active_calories")}
              value={activeCalories > 0 ? activeCalories.toString() : "--"}
              unit={t("personal_data.unit_kcal")}
              colors={colors}
              styles={styles}
            />
            ...
            <MetricCard
              ...
              label={t("personal_data.total_energy")}
            ...
            <MetricCard
              ...
              label={t("personal_data.sleep_duration")}
            ...
            <MetricCard
              ...
              label={t("personal_data.movement_time")}
              value={movementMinutes > 0 ? movementMinutes.toString() : "--"}
              unit={t("personal_data.unit_min")}
              colors={colors}
              styles={styles}
            />
          </View>
