  ...
  if (!article) {
    return (
      <View
        style={[styles.centerContainer, { backgroundColor: colors.background }]}
      >
        <Text style={{ color: colors.text }}>{t("article.not_found")}</Text>
        <TouchableOpacity
          onPress={() => router.back()}
          style={{ marginTop: 20 }}
        >
          <Text style={{ color: "#3b82f6" }}>{t("article.back")}</Text>
        </TouchableOpacity>
      </View>
    );
  }
    ...
    return blocks.map((block, index) => {
      if (block.startsWith("## ")) {
        return (
          <Text
            key={index}
            style={[styles.contentHeading2, { color: colors.text }]}
          >
            {block.replace("## ", "")}
          </Text>
        );
        ...
        return (
          <Text
            key={index}
            style={[styles.contentHeading3, { color: colors.text }]}
          >
            {block.replace("### ", "")}
          </Text>
        );
      ...
      return (
        <Text
          key={index}
          style={[styles.contentParagraph, { color: colors.textSecondary }]}
        >
          {block}
        </Text>
      );
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={styles.headerAbsolute}>
        ...
        <Text style={styles.headerTitle}>{t("article.details")}</Text>
        <TouchableOpacity
          ...
          <View style={styles.metaRow}>
            <View style={styles.categoryBadge}>
              <Text style={styles.categoryBadgeText}>
                {article.category?.toUpperCase() || t("article.health")}
              </Text>
            </View>
            <View style={styles.timeBadge}>
              <Ionicons name="time" size={14} color="#64748b" />
              <Text style={styles.timeBadgeText}>
                5 {t("article.read_time")}
              </Text>
            </View>
          </View>
          ...
          <View style={styles.authorRow}>
            ...
            <View>
              ...
              <Text style={styles.authorDesc}>
                {t("article.ai_expert")} • {formattedDate}
              </Text>
            </View>
          </View>
        </View>
