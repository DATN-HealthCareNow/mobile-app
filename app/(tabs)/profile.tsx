  ...
  return (
    ...
    >
      ...
      <View style={styles.header}>
        <View style={styles.logoRow}>
          ...
          <Text style={[styles.title, isDark && { color: "#f8fafc" }]}>
            ...
            <Text style={{ color: "#1497dd" }}>{t("profile.profile")}</Text>
          </Text>
        </View>
      ...
      <Text style={styles.name}>
        {profile?.fullName || profile?.full_name || t("settings.new_user")}
      </Text>
      ...
      {(profile?.subscriptionPlan === "PREMIUM" ||
        ...
        <View style={styles.badge}>
          ...
          <Text style={styles.badgeText}> {t("settings.premium")}</Text>
        </View>
      )}

      {/* BODY METRICS */}
      <View style={styles.sectionHeader}>
        <Text style={styles.sectionTitle}>{t("profile.body_metrics")}</Text>
        <TouchableOpacity onPress={() => router.push("/screen/metric_entry")}>
          <Text style={styles.updateText}>{t("profile.update")}</Text>
        </TouchableOpacity>
      </View>
      ...
      <View style={styles.grid}>
        ...
        <MetricCard
          ...
          value={
            profile?.gender
              ? t(`metrics.${profile.gender.toLowerCase()}`)
              : "--"
          }
          colors={colors}
          styles={styles}
      ...
      <AccountItem
        ...
        colors={colors}
        styles={styles}
        onPress={() => router.push("/screen/personal_data")}
      />
      <AccountItem
        icon="document-text-outline"
        title={t("profile.scan_medical_record")}
        colors={colors}
        styles={styles}
        onPress={() => router.push("/screen/medical_scan")}
      />
      <AccountItem
        icon="card-outline"
        title={t("profile.subscription_plan")}
        colors={colors}
        styles={styles}
        onPress={() => router.push("/screen/subscription")}
      />
      <AccountItem
